...
(define-map nft-legacy (buff 48) {last-child-id: (buff 16), family-size: (buff 32)})
...
(define-read-only (get-lv1-last-child-id (parent-id (buff 16))) 
...
(define-private (general-checks (lv uint) (recipient principal)) 
    (begin 
            ...
            (asserts! (not (is-eq (read-buff16 (var-get last-token-id) (* u16 lv)) FULL-16-BYTES)) (err ERR-CONTRACT-MAXED-OUT))
            (ok true))
)
...
(define-private (handle-lv0-mint (nft-uri (string-ascii 256)) (recipient principal))
    (let (
            ...
            (err-check (match (general-checks u0 recipient) response (ok response) error (err error)))
        )
        (asserts! (is-ok err-check) err-check)
        (var-set last-token-id (increment raw-token-id))
...
(define-public (mint (parent-id (optional (buff 48))) (nft-uri (string-ascii 256)) (recipient principal)) 
    (let 
        (   
            (nft-level (match parent-id id (- (/ (len id) u16) u1) u0))
        )
        ...
        (if (is-eq nft-level u0) (handle-lv0-mint nft-uri recipient) (err ERR-NOT-AUTHORIZED))
    )
)
...
(define-read-only (increment-closure (idx uint) (state {is-done: bool, data: (buff 16), acc: (buff 16)}))
    (let (
          (acc (get acc state))
          (data (get data state))
          (is-done (get is-done state))
          (byte (unwrap-panic (element-at data (- u15 idx))))
          (number (unwrap-panic (index-of BUFF_TO_BYTE byte)))
          (new-byte (if is-done
                        byte
                        (if (is-eq u255 number)
                            0x00
                            (unwrap-panic (element-at BUFF_TO_BYTE (+ u1 number))))))
          (done (or is-done (not (is-eq new-byte 0x00)))))

         (merge state { is-done: done, acc: (unwrap-panic (as-max-len? (concat new-byte acc) u16))})))





(define-read-only (increment (data (buff 16)))
    (get acc
